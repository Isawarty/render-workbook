...
void main() {
    ...
    vec3 direct = (kd * am.rgb / PI + specular) * vec3(4.5, 4.0, 3.5) * nl;
    ...
    vec3 diffuseIbl = ibl.values[0].rgb * am.rgb * (1.0 - am.a);
    vec3 specularIbl = ibl.values[prefilterIndex].rgb * (f0 * brdf.x + brdf.y);
    vec3 hdr = direct + (diffuseIbl + specularIbl) * ea.a + ea.rgb;
