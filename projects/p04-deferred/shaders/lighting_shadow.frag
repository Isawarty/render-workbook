...
void main() {
    ...
    vec3 direct = (kd * am.rgb / PI + specular) * vec3(4.5, 4.0, 3.5) * nl * visibility;
    ...
    vec3 hdr = direct + ambient * ea.a + ea.rgb;
    vec3 mapped = pow(hdr / (hdr + vec3(1.0)), vec3(1.0 / 2.2));
    // t05 把 PCF visibility 同时写进 alpha，供离屏判分直接观测；
    // 窗口 swapchain 使用 opaque composite，RGB 视觉结果不受影响。
    outColor = vec4(mapped, visibility);
}
