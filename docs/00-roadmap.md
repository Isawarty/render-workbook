...
| task | 内容 |
|---|---|
| t01 | 声明式的 pass / resource API（✅ 已实现） |
| t02 | 有向图构建 + 拓扑排序（✅ 已实现） |
| t03 | **自动 barrier 推导**（读写状态机，✅ 已实现并接入 Vulkan） |
| t04 | 资源生命周期分析 + transient 资源别名（✅ 真实 VMA allocation 复用） |
| t05 | 用 render graph 重写 P4 的延迟管线（✅ 保持 P4 交互与画面） |
| t06 | 图可视化（✅ Graphviz 导出 + **ImGui 面板**） |

图算法部分是纯 CPU 逻辑，可以写高覆盖单测；端到端验收是
「渲染结果与 P4 的 golden image 一致」。

P5 的 t01–t06 已发布；窗口默认保持 P4 漫游，按 `F1` 打开图面板并释放鼠标。
...
**t00 语言速通（10–15h，无挖空，✅ 已交付）**：20 道纯 Lua 小题，覆盖 table / metatable /
闭包 / 协程 / 模块，自带 runner 判分。约 2 周。

| task | 内容 |
|---|---|
| t01 | `lua_State` 嵌入 + 栈操作 + 错误处理（✅ 已交付） |
| t02 | C 函数注册 + userdata + metatable（向 Lua 暴露 vec3 / mat4，✅ 已交付） |
| t03 | 用 Lua 描述材质与场景（✅ 已交付） |
| t04 | **用 Lua 声明 render graph**（接 P5，✅ 已交付） |
| t05 | 热重载 + 沙箱 + 协程调度（✅ 已交付） |
